...
http.createServer((req, res) => {
  if (req.url === '/health') { res.writeHead(200, {'content-type':'text/plain'}); res.end('La Silent Aura bridge is running'); return }
  res.writeHead(200, {'content-type':'text/plain'}); res.end('La Silent Aura Telegram ↔ WhatsApp bridge');
...
async function sendTelegramMedia(kind, buffer, caption = '', filename = 'file.bin') {
  ...
  const response = await fetch(api(kind === 'photo' ? 'sendPhoto' : kind === 'video' ? 'sendVideo' : kind === 'audio' ? 'sendAudio' : kind === 'voice' ? 'sendVoice' : 'sendDocument'), {
    method: 'POST',
    body: form,
  })
  const data = await response.json()
  if (!data.ok) throw new Error(`Telegram media: ${data.description || 'request failed'}`)
}
...
function senderName(message) {
  return message?.pushName || message?.key?.participant?.split('@')[0] || message?.key?.remoteJid?.split('@')[0] || 'WhatsApp'
}
...
async function forwardWhatsAppToTelegram(message) {
  ...
  try {
    ...
    const filename = message.message?.documentMessage?.fileName || `${kind}-${Date.now()}`
    await sendTelegramMedia(kind, buffer, `${prefix}${text ? `\n${text}` : ''}`, filename)
  } catch (error) {
    console.error('WhatsApp → Telegram media error:', error?.message || error)
    if (text) await sendTelegramText(`${prefix}\n${text}`)
  }
}

async function forwardTelegramToWhatsApp(update) {
  if (!waSocket || !WHATSAPP_GROUP_ID || BRIDGE_MODE === 'whatsapp_to_telegram') return
  const msg = update?.message
  if (!msg) return
  if (!telegramChatId) {
    telegramChatId = String(msg.chat.id)
    console.log(`TELEGRAM_CHAT_ID discovered: ${telegramChatId}`)
  } else if (String(msg.chat.id) !== telegramChatId) return

  const caption = msg.caption || ''
  if (msg.text) {
    await waSocket.sendMessage(WHATSAPP_GROUP_ID, { text: `╭─『 𝓛𝓪 𝓢𝓲𝓵𝓮𝓷𝓽 𝓐𝓾𝓻𝓪 』─╮\n${msg.text}\n╰────────────────────╯` })
    return
  }
  ...
  try {
    if (msg.photo?.length) {
      ...
      await waSocket.sendMessage(WHATSAPP_GROUP_ID, { image: buffer, caption })
      return
    }
    if (msg.video) {
      const buffer = await tgFile(msg.video.file_id)
      await waSocket.sendMessage(WHATSAPP_GROUP_ID, { video: buffer, caption, mimetype: msg.video.mime_type || 'video/mp4' })
      return
    }
    if (msg.audio) {
      const buffer = await tgFile(msg.audio.file_id)
      await waSocket.sendMessage(WHATSAPP_GROUP_ID, { audio: buffer, mimetype: msg.audio.mime_type || 'audio/mpeg', ptt: false })
      return
    }
    if (msg.voice) {
      const buffer = await tgFile(msg.voice.file_id)
      await waSocket.sendMessage(WHATSAPP_GROUP_ID, { audio: buffer, mimetype: msg.voice.mime_type || 'audio/ogg; codecs=opus', ptt: true })
      return
    }
    if (msg.document) {
      const buffer = await tgFile(msg.document.file_id)
      await waSocket.sendMessage(WHATSAPP_GROUP_ID, { document: buffer, fileName: msg.document.file_name || 'document' , mimetype: msg.document.mime_type || 'application/octet-stream', caption })
    }
  } catch (error) {
    console.error('Telegram → WhatsApp media error:', error?.message || error)
  }
}
...
async function telegramLoop() {
  ...
  while (true) {
    try {
      const updates = await tg('getUpdates', { offset: telegramOffset, timeout: 25, allowed_updates: ['message'] })
...
async function startWhatsApp() {
  ...
  waSocket.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
    if (connection === 'open') {
      ...
      if (WHATSAPP_GROUP_ID) console.log(`WhatsApp target group: ${WHATSAPP_GROUP_ID}`)
      else console.log('WHATSAPP_GROUP_ID is empty — send a message in your target group to discover its JID.')
    }

    if (connection === 'close') {
      const code = lastDisconnect?.error?.output?.statusCode
      console.log(`WhatsApp disconnected (${code ?? 'unknown'})`)
      if (code !== DisconnectReason.loggedOut) {
        clearTimeout(reconnectTimer)
        reconnectTimer = setTimeout(() => startWhatsApp().catch(console.error), 5000)
      } else {
        console.error('WhatsApp session logged out. Delete bridge_data/whatsapp-session and pair again.')
      }
    }
  })

  if (!state.creds.registered && WHATSAPP_PHONE_NUMBER && !pairingPrinted) {
    pairingPrinted = true
    await sleep(1500)
    try {
      const code = await waSocket.requestPairingCode(WHATSAPP_PHONE_NUMBER)
  ...
  waSocket.ev.on('messages.upsert', async ({ messages }) => {
    for (const message of messages || []) {
      ...
      const text = extractWaText(message).trim().toLowerCase()
      if (jid.endsWith('@g.us') && text === '.menu' && !message.key?.fromMe && WHATSAPP_GROUP_ID && jid === WHATSAPP_GROUP_ID) {
        try {
          if (fs.existsSync(MENU_IMAGE)) {
            await waSocket.sendMessage(jid, {
              image: fs.readFileSync(MENU_IMAGE),
              caption: '╰┈➤ 𓆩⟡『 𝓛𝓪 𝓢𝓲𝓵𝓮𝓷𝓽 𝓐𝓾𝓻𝓪 』⟡𓆪\n\n𝑷𝑹𝑬𝑴𝑰𝑼𝑴 𝑴𝑼𝑳𝑻𝑰 𝑫𝑬𝑽𝑰𝑪𝑬',
            })
          } else {
            await waSocket.sendMessage(jid, { text: '╰┈➤ 𓆩⟡『 𝓛𝓪 𝓢𝓲𝓵𝓮𝓷𝓽 𝓐𝓾𝓻𝓪 』⟡𓆪' })
          }
        } catch (error) {
      ...
      await forwardWhatsAppToTelegram(message).catch((error) => console.error('WA → TG error:', error?.message || error))

      // Useful first-run discovery when no group ID has been configured.
      if (jid.endsWith('@g.us') && !WHATSAPP_GROUP_ID) {
        console.log(`DISCOVERED WHATSAPP GROUP: ${jid}`)
      }
    }
  })
}
...
console.log('╭────────────────────────────────────────────╮')
console.log('│  𝓛𝓪 𝓢𝓲𝓵𝓮𝓷𝓽 𝓐𝓾𝓻𝓪 — Telegram ↔ WhatsApp  │')
console.log('╰────────────────────────────────────────────╯')
console.log(`Bridge mode: ${BRIDGE_MODE}`)
console.log(`Telegram chat: ${telegramChatId || 'AUTO (first incoming chat)'}`)

await Promise.all([startWhatsApp(), telegramLoop()])
